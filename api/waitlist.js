...
// Per-instance only, so the effective cap is N × RATE_MAX. Distributed
// limit kicks in automatically once the Upstash Marketplace integration
// is installed (no redeploy needed).
...
async function rateLimited(ip) {
  const r = await rateLimit(`waitlist:${ip}`, RATE_MAX, RATE_WINDOW_MS)
